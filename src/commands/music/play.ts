...
import { LoadType, PlaylistInfo } from '@skyra/audio';
import * as path from 'path';
import * as url from 'url';
import { EMOJIS } from '../../lib/util/Constants';
import { timeString } from '../../lib/util/timeString';
...
export default class PlayCommand extends Command {
  ...
  public async exec(
    message: Message,
    { query, unshift }: { query: string; unshift: boolean }
  ) {
    if (!message.member?.voice?.channel) {
      return message.util!.send({
        embed: {
          description: `${EMOJIS.error} You Need to be in voice channel to use that command!`,
          color: 'RED',
        },
      });
      ...
      return message.util!.reply({
        embed: {
          description: `${EMOJIS.error} I do not have permissions to connect to that channel!`,
          color: 'RED',
        },
      });
      ...
      return message.util!.reply({
        embed: {
          description: `${EMOJIS.error} I do not have permissions to speak in that channel!`,
          color: 'RED',
        },
      });
    }
    ...
    if ([LoadType.TrackLoaded, LoadType.SearchResult].includes(res.loadType)) {
      if (unshift) await queue.unshift(res.tracks[0].track);
      else await queue.add(res.tracks[0].track);
      msg = this.client.util
        .embed()
        .setColor('#ffa053')

        .setThumbnail(
          `https://i.ytimg.com/vi/${res.tracks[0].info.identifier}/hqdefault.jpg`
        )
        ...
        .addField(
          'Length',
          `(${
            res.tracks[0].info.isStream
              ? '∞'
              : timeString(res.tracks[0].info.length)
          })`,
          true
        )
    ...
    } else if (res.loadType === LoadType.PlaylistLoaded) {
      await queue.add(...res.tracks.map((track) => track.track));
      msg = (res.playlistInfo as PlaylistInfo).name;
    } else {
      return message.util!.send({
        embed: {
          description: `${EMOJIS.error} I couldn'nt look for the song you are trying to search for!`,
          color: 'RED',
        },
      });
    }
